...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        self.ool_embedding = torch.zeros(self.dim, dtype=torch.float32)
        self.epochs = epochs
    ...
    def log_prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        ...
        # Retrieve embeddings
        x_embed = self.lexicon.embeddings[x_idx] if x_idx else self.ool_embedding
        y_embed = self.lexicon.embeddings[y_idx] if y_idx else self.ool_embedding
        z_embed = self.lexicon.embeddings[z_idx] if z_idx else self.ool_embedding

        # Compute numerator and denominator
        numerator = x_embed @ self.X @ z_embed + y_embed @ self.Y @ z_embed
        
        E = torch.stack([self.lexicon.embeddings[self.lexicon.vocab.index(word)] 
                         if self.lexicon.vocab.index(word) else self.ool_embedding
                         for word in self.vocab
                         ], dim=1)  # Stack along dimension 1 to create columns
        xX = x_embed @ self.X
        yY = y_embed @ self.Y
        ...
        # print(f"shape of E: {E.shape}, should be 10, 75")
        # print(f"shape of xX: {xX.shape}, should be 10")
        # print(f"shape of yY: {xX.shape}, should be 10")

        denominator = torch.logsumexp(xX @ E + yY @ E, dim = 0)
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        return torch.tensor(self.log_prob(x, y, z))
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor,"vocab"]:
        ...
        x_idx = self.lexicon.vocab.index(x) if x in self.lexicon.vocab else None
        y_idx = self.lexicon.vocab.index(y) if y in self.lexicon.vocab else None
        ...
        # Transpose so embeddings are in columns
        E = self.lexicon.embeddings.transpose(0, 1)  # Shape: (embedding_dim, vocab_size)
        xX = x_embed @ self.X  # Shape: (embedding_dim,)
        yY = y_embed @ self.Y  # Shape: (embedding_dim,)

        # Compute the logits for all z' in the vocabulary
        # (x^T X z' + y^T Y z') for all z'
        logits = xX @ E + yY @ E  # Resulting shape: (vocab_size,)
